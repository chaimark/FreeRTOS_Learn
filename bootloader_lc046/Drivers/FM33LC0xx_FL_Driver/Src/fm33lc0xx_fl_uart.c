...
#define IS_UART_INSTANCE(INSTANCE)                                                                                     \
    (((INSTANCE) == UART0) || ((INSTANCE) == UART1) || ((INSTANCE) == UART4) || ((INSTANCE) == UART5))

#define IS_FL_UART_CLKSRC(__VALUE__)                                                                                   \
    (((__VALUE__) == FL_RCC_UART1_CLK_SOURCE_APB1CLK) || ((__VALUE__) == FL_RCC_UART1_CLK_SOURCE_RCHF) ||              \
     ((__VALUE__) == FL_RCC_UART1_CLK_SOURCE_SYSCLK) || ((__VALUE__) == FL_RCC_UART1_CLK_SOURCE_RCMF_PSC) ||           \
     ((__VALUE__) == FL_RCC_UART0_CLK_SOURCE_APB1CLK) || ((__VALUE__) == FL_RCC_UART0_CLK_SOURCE_RCHF) ||              \
     ((__VALUE__) == FL_RCC_UART0_CLK_SOURCE_SYSCLK) || ((__VALUE__) == FL_RCC_UART0_CLK_SOURCE_RCMF_PSC))


#define IS_FL_UART_DATAWIDTH(__VALUE__)                                                                                \
    (((__VALUE__) == FL_UART_DATA_WIDTH_7B) || ((__VALUE__) == FL_UART_DATA_WIDTH_8B) ||                               \
     ((__VALUE__) == FL_UART_DATA_WIDTH_9B) || ((__VALUE__) == FL_UART_DATA_WIDTH_6B))

#define IS_FL_UART_STOPBITS(__VALUE__)                                                                                 \
    (((__VALUE__) == FL_UART_STOP_BIT_WIDTH_1B) || ((__VALUE__) == FL_UART_STOP_BIT_WIDTH_2B))

#define IS_FL_UART_PARITY(__VALUE__)                                                                                   \
    (((__VALUE__) == FL_UART_PARITY_NONE) || ((__VALUE__) == FL_UART_PARITY_EVEN) ||                                   \
     ((__VALUE__) == FL_UART_PARITY_ODD))

#define IS_FL_UART_DIRECTION(__VALUE__)                                                                                \
    (((__VALUE__) == FL_UART_DIRECTION_NONE) || ((__VALUE__) == FL_UART_DIRECTION_RX) ||                               \
     ((__VALUE__) == FL_UART_DIRECTION_TX) || ((__VALUE__) == FL_UART_DIRECTION_TX_RX))

#define IS_FL_UART_INFRA_MODULATION(__VALUE__) (((__VALUE__) == FL_DISABLE) || ((__VALUE__) == FL_ENABLE))


#define IS_FL_UART_INFRARED_POLARITY(__VALUE__)                                                                        \
    (((__VALUE__) == FL_UART_INFRARED_POLARITY_NORMAL) || ((__VALUE__) == FL_UART_INFRARED_POLARITY_INVERT))
...
FL_ErrorStatus FL_UART_InfraRed_Init(UART_Type* UARTx, FL_UART_InfraRed_InitTypeDef* initStruct) {
    ...
    /* 调制占空比 */
    if ((tempTZBRG >> 4) != 0) {
        tempTH = (uint32_t)(((float)initStruct->modulationDuty / 100.0) *
                                ((float)(tempTZBRG + 1) / (float)(tempTZBRG >> 4)) +
                            0.5);
    } else {
