...
uint16_t _Module_TXD_6844_GetTx(void) {
    ...
    //     SendDataLen = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, RoomTemperArray_TAG, 0x01); //
    //     添加汇云数据结构 室温数组
    // }
    SendDataLen = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, ValveSturt, 0x01); // 添加汇云数据结构
    // SendDataLen = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, nowTime_TAG, 0x03); //
    // 添加汇云数据结构 SendDataLen = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, voltage_TAG,
    // 0x01); // 添加汇云数据结构 SendDataLen = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen,
    // CCID_TAG, 0x01); // 添加汇云数据结构 SendDataLen = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf),
    // SendDataLen, IMEI_TAG, 0x01); // 添加汇云数据结构 SendDataLen =
    // Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, CSQ_TAG, 0x01); // 添加汇云数据结构 SendDataLen =
    // Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, RSRP_TAG, 0x01); // 添加汇云数据结构 SendDataLen
    // = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, RSRQ_TAG, 0x01); // 添加汇云数据结构

    if ((SendManageObj_Count == 0) || (_Module_Start_Flage == _Module_UserSend) ||
        (_Module_Start_Flage == _Module_EEpromSend)) {
        SendManageObj_Count = MaxSendCount_ManageObj;
        SendDataLen         = Add_HY_DataClass(NEW_NAME(_Module_TXD_6844_TxBuf), SendDataLen, SendClassObj_TAG,
                                               0x01); // 添加汇云数据结构 0x01 上传参数
    } else {
        SendManageObj_Count--;
    }
