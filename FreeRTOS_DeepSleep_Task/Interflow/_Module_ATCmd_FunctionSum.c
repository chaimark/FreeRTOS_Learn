...
NetDevATCmd NetDevice_ATData__NB[ATCMD_MAXNUMBER] = {
    {0, 1, "AT+IPR=115200\r\n", "OK", true, 3, 2, 1000, false, setUartBandDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //设置波特率
    {1, 2, "AT+SGSW\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL, NULL},
    {2, 3, "AT+ECBAND=0,5\r\n", "OK", true, 3, 2, 1000, false, setSIMBandDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //设置 SIM BAND
    {3, 4, "AT+CGSN\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, get_Module_DevIMEIFun, NULL,
     NULL}, //请求产品序列号IMEI
    {4, 5, "AT+ECICCID\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, get_Module_DevCCIDFun, NULL,
     NULL}, //获取CCID
    {5, 6, "AT+CPIN?\r\n", "READY", true, 3, 2, 2000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //识别sm卡
    {6, 7, "AT+QSCLK=0\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //关闭自动休眠锁
    {7, 8, "AT+CSCON?\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //信令连接状态
    {8, 9, "AT+NNMI=2\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //关闭下行数据回显
    {9, 10, "AT+CEREG?\r\n", "CEREG: 0,1", true, 10, 1, 1000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL},
    {10, 11, "AT+CFUN=1\r\n", "OK", true, 3, 2, 2000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //开启射频
    {11, 12, "AT+CSQ\r\n", "OK", true, 3, 2, 3000, false, easyATCmdDataLoadFun, get_Module_DevCSQFun, NULL,
     NULL}, //获取信号质量
    {12, 13, "AT+CESQ\r\n", "OK", true, 3, 2, 3000, false, easyATCmdDataLoadFun, get_Module_DevCESQFun, NULL,
     NULL}, //获得扩展的信号质量
    {13, 14, "AT+CIMI\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, get_Module_DevIMSIFun, NULL,
     NULL}, //获得SIM卡的IMSI
    {14, 15, "AT+CGPADDR?\r\n", "OK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //显示PDP地址
    {15, 16, "AT+NCFG=0,86400\r\n", "OK", true, 3, 2, 8000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //如果对接 AEP 平台, 必须要配置AT+NCFG=0,<lifetime>, 否则无法连接上 AEP 平台
    {16, 17, "AT+NCDPOPEN=\"221.229.214.202\",5683\r\n", "OK", true, 3, 2, 3000, false, setRemoteIPDataLoadFun,
     get_Module_IPLinkFun, NULL, NULL}, //连接到目标IP
    {17, 18, "AT+CCLK?\r\n", "+CCLK", true, 3, 2, 1000, false, easyATCmdDataLoadFun, get_Module_DevCCLKFun, NULL,
     NULL}, //返回当前日期和时间
    {18, -1, "AT+NMGR\r\n", "OK", false, 3, 2, 5000, false, easyATCmdDataLoadFun, easyATCmdDataDoingFun, NULL,
     NULL}, //获取下行数据
...
void setSIMBandCmdDataLoad(strnew OutStr, NetDevATCmd NowATCmd) {
    memset(OutStr.Name._char, 0, OutStr.MaxLen);
    // sprintf(OutStr.Name._char, "AT+ECBAND=0,%d\r\n", (AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_BAND != 0) ?
    // AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_BAND : IMSI_BAND);
...
void setRemoteIPCmdDataLoad(strnew OutStr, NetDevATCmd NowATCmd) {
    memset(OutStr.Name._char, 0, OutStr.MaxLen);
    sprintf(OutStr.Name._char, "AT+NCDPOPEN=\"%s\",%d\r\n", AT24CXX_Manager_NET.NET_Remote_Url,
            AT24CXX_Manager_NET.NET_Remote_Port);
    int ArrayLen                                                                   = strlen(OutStr.Name._char);
    OutStr.Name._char[(ArrayLen < OutStr.MaxLen) ? ArrayLen : (OutStr.MaxLen - 1)] = '\0';
}
...
bool get_Module_DevIMEI(NetDevATCmd NowATCmd) {
    ...
    if ((StartAddr = myStrstr(CmdTable.RxBuf, "\r\n\r\nOK\r\n", CmdTable.RxLen)) != NULL) {
        ...
        // 比较 IMEI 是否相同
        Temp_isWriteEEprom |=
            ((strcmp(StartAddr, AT24CXX_Manager_NET.Get_Module_Data._Module_DEV_IMEI) == 0) ? false
                                                                                            : true); // 如果相同则不写入
    ...
    memset(AT24CXX_Manager_NET.Get_Module_Data._Module_DEV_IMEI, 0,
           ARR_SIZE(AT24CXX_Manager_NET.Get_Module_Data._Module_DEV_IMEI));
    return false;
}
// 获取 IMSI
bool get_Module_DevIMSI(NetDevATCmd NowATCmd) {
    ...
    if ((StartAddr = myStrstr(CmdTable.RxBuf, "\r\n\r\nOK\r\n", CmdTable.RxLen)) != NULL) {
        ...
        // 如果相同则不写入
        setIsWriteEEprom(((strcmp(StartAddr, AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_IMSI) == 0)
                              ? 0xFF
                              : true)); // 如果相同则不写入
        strcpy(AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_IMSI, StartAddr);
        return true;
    }
    memset(AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_IMSI, 0,
           ARR_SIZE(AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_IMSI));
    return false;
}
// 获取 CCID
bool get_Module_DevCCID(NetDevATCmd NowATCmd) {
    ...
    if ((StartAddr = myStrstr(CmdTable.RxBuf, "+ECICCID: ", CmdTable.RxLen)) != NULL) {
        ...
        char* EndAddr = NULL;
        if ((EndAddr = strchr(StartAddr, '\r')) == NULL) {
            memset(AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_ICCID, 0,
                   ARR_SIZE(AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_ICCID));
            return false;
        }
        (*EndAddr) = '\0';
        // 如果相同则不写入
        setIsWriteEEprom(
            ((strcmp(StartAddr, AT24CXX_Manager_NET.Get_Module_Data._Module_SIM_ICCID) == 0) ? 0xFF : true));
...
bool get_Module_DevCCLK(NetDevATCmd NowATCmd) {
    if ((_Module_Start_Flage == _Module_UserSend) || (Min_15_TASK.Task[DayOverCclkTask].TimeTask_Falge)) {
        Min_15_TASK.InitSetTimeTask(DayOverCclkTask, HourToMin_15(AT24CXX_Manager_NET.DaysNumberOfCCLK * 24),
                                    NULL); // 604,800
    } else {
        return true;
    }
    ...
    for (int MaxCount_i = 0; MaxCount_i < 3; MaxCount_i++) { // 三次校时机会
        ...
        for (uint8_t i = 0; i < 2; i++) { // 任意一次两时间相同
            ...
            if ((StartAddr = myStrstr(CmdTable.RxBuf, "+CCLK: ", CmdTable.RxLen)) != NULL) {
                int Min15Cont = 0;
                sscanf(StartAddr, "+CCLK: %d/%d/%d,%d:%d:%d+%d", &_Module_Time_Data.year, &_Module_Time_Data.month,
                       &_Module_Time_Data.day, &_Module_Time_Data.hour, &_Module_Time_Data.minute,
                       &_Module_Time_Data.second, &Min15Cont);
                _Module_Time_Data = RTCTime_Hex_AddSecToNewTime(_Module_Time_Data, (Min15Cont * 15 * 60));

                _Module_Time_Data.year  = (uint32_t)anyBaseToAnyBase((uint64_t)_Module_Time_Data.year, 16,
                                                                     10); // 将 _Module_Time_Data.year 16进制转换成10进制
                _Module_Time_Data.month = (uint32_t)anyBaseToAnyBase(
                    (uint64_t)_Module_Time_Data.month, 16, 10); // 将 _Module_Time_Data.month 16进制转换成10进制
                _Module_Time_Data.day    = (uint32_t)anyBaseToAnyBase((uint64_t)_Module_Time_Data.day, 16,
                                                                      10); // 将 _Module_Time_Data.day 16进制转换成10进制
                _Module_Time_Data.hour   = (uint32_t)anyBaseToAnyBase((uint64_t)_Module_Time_Data.hour, 16,
                                                                      10); // 将 _Module_Time_Data.hour 16进制转换成10进制
                _Module_Time_Data.minute = (uint32_t)anyBaseToAnyBase(
                    (uint64_t)_Module_Time_Data.minute, 16, 10); // 将 _Module_Time_Data.minute 16进制转换成10进制
                _Module_Time_Data.second = (uint32_t)anyBaseToAnyBase(
                    (uint64_t)_Module_Time_Data.second, 16, 10); // 将 _Module_Time_Data.second 16进制转换成10进制
                _Module_Time_Data.week =
                    getDayOfWeek(_Module_Time_Data.year, _Module_Time_Data.month, _Module_Time_Data.day);
    ...
    if (isTimeOK) {
        ...
        sprintf(NowTimeData.Name._char, "%02x-%02x-%02x %02x:%02x:%02x", TempStringOfTime[1].year,
                TempStringOfTime[1].month, TempStringOfTime[1].day, TempStringOfTime[1].hour,
                TempStringOfTime[1].minute, TempStringOfTime[1].second);
