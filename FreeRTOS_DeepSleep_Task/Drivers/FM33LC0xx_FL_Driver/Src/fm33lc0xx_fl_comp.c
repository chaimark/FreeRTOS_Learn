...
#define IS_COMP_ALL_INSTANCE(INTENCE) (((INTENCE) == COMP1) || ((INTENCE) == COMP2))

#define IS_FL_COMP_POSITIVEINPUT(__VALUE__)                                                                            \
    (((__VALUE__) == FL_COMP_INP_SOURCE_INP1) || ((__VALUE__) == FL_COMP_INP_SOURCE_INP2))

#define IS_FL_COMP_NEGATIVEINPUT(__VALUE__)                                                                            \
    (((__VALUE__) == FL_COMP_INN_SOURCE_INN1) || ((__VALUE__) == FL_COMP_INN_SOURCE_INN2) ||                           \
     ((__VALUE__) == FL_COMP_INN_SOURCE_VREF) || ((__VALUE__) == FL_COMP_INN_SOURCE_VREF_DIV_2))

#define IS_FL_COMP_POLARITY(__VALUE__)                                                                                 \
    (((__VALUE__) == FL_COMP_OUTPUT_POLARITY_NORMAL) || ((__VALUE__) == FL_COMP_OUTPUT_POLARITY_INVERT))

#define IS_FL_COMP_EDGE(__VALUE__)                                                                                     \
    (((__VALUE__) == FL_COMP_INTERRUPT_EDGE_BOTH) || ((__VALUE__) == FL_COMP_INTERRUPT_EDGE_RISING) ||                 \
     ((__VALUE__) == FL_COMP_INTERRUPT_EDGE_FALLING))

#define IS_FL_COMP_DIGITAL_FILTER(__VALUE__) (((__VALUE__) == FL_DISABLE) || ((__VALUE__) == FL_ENABLE))
...
FL_ErrorStatus FL_COMP_Init(COMP_Type* COMPx, FL_COMP_InitTypeDef* initStruct) {
    ...
    if ((initStruct->negativeInput == FL_COMP_INN_SOURCE_VREF) ||
        (initStruct->negativeInput == FL_COMP_INN_SOURCE_VREF_DIV_2)) {
