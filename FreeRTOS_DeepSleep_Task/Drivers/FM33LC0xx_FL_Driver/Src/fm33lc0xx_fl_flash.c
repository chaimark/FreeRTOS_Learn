...
#define IS_FL_FLASH_PAGE_NUM(__VALUE__)                                                                                \
    ((((uint32_t)0x00000000U) < (__VALUE__)) && ((__VALUE__) <= (FL_FLASH_MAX_PAGE_NUM)))

#define IS_FL_FLASH_SECTOR_NUM(__VALUE__)                                                                              \
    ((((uint32_t)0x00000000U) < (__VALUE__)) && ((__VALUE__) <= (FL_FLASH_MAX_SECTOR_NUM)))

#define IS_FL_FLASH_MAX_ADDR(__VALUE__)                                                                                \
    ((((uint32_t)0x00000000U) < (__VALUE__)) && ((__VALUE__) <= (FL_FLASH_ADDR_MAXPROGRAM)))

#define IS_FL_FLASH_MAX_PAGE(__VALUE__)                                                                                \
    ((((uint32_t)0x00000000U) < (__VALUE__)) && ((__VALUE__) <= (FL_FLASH_MAX_PAGE_NUM)))

#define IS_FL_FLASH_MAX_SECTOR(__VALUE__)                                                                              \
    ((((uint32_t)0x00000000U) < (__VALUE__)) && ((__VALUE__) <= (FL_FLASH_MAX_SECTOR_NUM)))
/**
 * @}
 */
...
FL_ErrorStatus FL_FLASH_PageErase(FLASH_Type* FLASHx, uint32_t address) {
    ...
    *((uint32_t*)address) = FL_FLASH_ERASE_REQUEST;
    while (1) {
        timeout++;
        if ((timeout > FL_FLASH_ERASE_TIMEOUT) || (FL_FLASH_IsActiveFlag_ClockError(FLASHx)) ||
            (FL_FLASH_IsActiveFlag_KeyError(FLASHx)) || (FL_FLASH_IsActiveFlag_AuthenticationError(FLASHx))) {
            /* 超时或出现错误 */
            ret = FL_FAIL;
            break;
