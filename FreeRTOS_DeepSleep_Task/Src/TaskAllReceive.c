...
void SetRTCtimeCard(strnew SetRTCTime) {
    ...
    sprintf(NowTimeData.Name._char, "20%02x-%02x-%02x %02x:%02x:%02x", SetRTCTime.Name._char[0],
            SetRTCTime.Name._char[1], SetRTCTime.Name._char[2], SetRTCTime.Name._char[3], SetRTCTime.Name._char[4],
...
void LimitCard(strnew SetLimitTime, uint16_t SetDegree) {
    ...
    sprintf(NowTimeStr.Name._char, "20%02x-%02x-%02x %02x:%02x:%02x", NowYear, NowMonth, NowDay, NowHour, NowMinute,
            NowSecond);
    uint32_t NowTime = getTimeNumber_UTCByRTCTime(NowTimeStr);
    memset(NowTimeStr.Name._char, 0, NowTimeStr.MaxLen);
    sprintf(NowTimeStr.Name._char, "20%02x-%02x-%02x 00:00:00", SetLimitTime.Name._char[0], SetLimitTime.Name._char[1],
...
bool DoneCardCmd(UserCard* _UserCardData) {
    ...
    if (!(UserCardData.MeterID[0] == 0xFF) && (UserCardData.MeterID[1] == 0xFF) && (UserCardData.MeterID[2] == 0xFF) &&
        (UserCardData.MeterID[3] == 0xFF)) {
