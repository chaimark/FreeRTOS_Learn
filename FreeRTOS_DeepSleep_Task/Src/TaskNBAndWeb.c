...
void StartNB_SUB(void) {
    CloseNB_Timer = xTimerCreate("CloseNB_Time",      // 定时器名称
    ...
    SendCount++; // 发送次数
    // ************************************ 判断当天属于什么模式 ************************************ //
    char NowSendFlagMode =
        readNowSendMode(daysFormOneMonthOneDay(GetDateHex(NowYear), GetDateHex(NowMonth), GetDateHex(NowDay)));
    // ************************************ 判断当天属于什么模式 ************************************ //
...
void NBModeTask(void* pvParameters) {
    ...
    while (1) {
        ...
        /* 永久等待任意事件位 */
        EventBits_t uxBits = xEventGroupWaitBits(xEventGroup_NB,
