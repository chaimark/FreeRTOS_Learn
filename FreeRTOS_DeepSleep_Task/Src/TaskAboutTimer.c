...
#define AllxEvent_Timer                                                                                                \
    (xEvent_TestBatV | xEvent_TestTempe | xEvent_TestValve | xEvent_yuliu1 | xEvent_checkTimeLimit | xEvent_yuliu2 |   \
     xEvent_ValveAutoTest | xEvent_yuliu3 | xEvent_yuliu4 | xEvent_yuliu5 | xEvent_TestAlarm)
...
void startTimeTask(void) {
    ...
    if (readDataBit(AT24CXX_Manager_NET.ModeCode, Enable_NBMode)) {
        ...
        if ((RTC_TASK.Task[SendIntervalTask].TimeTask_Falge) &&
            (System_RunData.Now_NetDevParameter.isWriteEEprom == false)) { // _Module_ 发送判断机制
            _Module_Start_Flage = _Module_AutoSend;
    ...
    if (CallCount % 5 == 0) {
        xEventGroupSetBitsFromISR(xEventGroup_TimeTask, xEvent_TestAlarm,
                                  &xHPW_TaskWoken); // 设置标志位, 检测是否存在报警信息
    }
    CallCount++;
    // 任意一个事件发生则释放信号量
    if ((System_RunData.isPowerModuleUseing != true) &&
        (xEventGroupGetBitsFromISR(xEventGroup_TimeTask) & AllxEvent_Timer) != 0) {
        // 发出信号量, 准备继续运行
...
void startRunTask(TimerHandle_t xTimer) {
    ...
    if (System_RunData.Now_Degree_Part == DegreePartToUint(0) ||
...
void TimeTask(void* pvParameters) {
    ...
    TimerHandle_t StartSend433Timer = xTimerCreate("StartSend433",     // 定时器名称
    ...
    do {
        ...
        if (xEventGroupGetBits(xEventGroup_TimeTask) & xEvent_checkTimeLimit) {
            ...
            sprintf(NowTimeStr.Name._char, "20%02x-%02x-%02x %02x:%02x:%02x", NowYear, NowMonth, NowDay, NowHour,
                    NowMinute, NowSecond);
