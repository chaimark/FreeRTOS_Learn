...
int main(void) {
    ...
    StartOPenDevMode();
    CheckMeterNum();
    xTaskCreate(WriteEEprom, "WriteEEprom", configMINIMAL_STACK_SIZE * 2, NULL, WriteEEprom_PRIORITY,
                &WriteEEpromHand); // 5 -- x
    xTaskCreate(ValveCtrlTask, "ValveCtrl", configMINIMAL_STACK_SIZE * 2, NULL, ValveCtrl_PRIORITY,
                &ValveCtrlHand);                                                                             // 5 -- x
    xTaskCreate(GpioTask, "GpioTask", configMINIMAL_STACK_SIZE * 2, NULL, GpioTask_PRIORITY, &GpioTaskHand); // 4 -- x
    xTaskCreate(AllReceive, "AllReceive", configMINIMAL_STACK_SIZE * 3, NULL, AllReceive_PRIORITY,
                &AllReceive_Hand);                                                                           // 3 -- x
    ...
    xTaskCreate(ReadEEprom, "ReadEEprom", configMINIMAL_STACK_SIZE * 2, NULL, ReadEEprom_PRIORITY,
                &ReadEEpromHand); // 1 -- x
    vTaskStartScheduler();
    while (1) {
        Sleep();
