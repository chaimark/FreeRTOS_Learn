...
int Add_HY_DataClass(strnew OutPutBuff, int startAddr, unsigned char DataTAG, unsigned char VIF) {
    ...
    case FlagOfRights:
        ...
        break;
    case IPAndProt:
        sprintf(&OutPutBuff.Name._char[startAddr], "%s:%d", AT24CXX_Manager_NET.NET_Remote_Url,
    ...
    case ValveSturt: {
        ...
        EEprom_Read_Start((unsigned char*)&RunTimeNum,
                          EEPROM_OTHER_ADDRESS + (AT24CXX_Manager_NET.RunTimeBack_AddrNum * 4), 4);
...
bool WeiteCmdSubData(strnew CmdSub) {
    ...
    */
    ...
    if (CmdSub.Name._char[3] == nowTime_TAG) {
        ...
        AT24CXX_Manager_NET.MeterID[0] =
            (((unsigned char)CmdSub.Name._char[5] != 0xFF) ? CmdSub.Name._char[5] : AT24CXX_Manager_NET.MeterID[0]);
        AT24CXX_Manager_NET.MeterID[1] =
            (((unsigned char)CmdSub.Name._char[6] != 0xFF) ? CmdSub.Name._char[6] : AT24CXX_Manager_NET.MeterID[1]);
        AT24CXX_Manager_NET.MeterID[2] =
            (((unsigned char)CmdSub.Name._char[7] != 0xFF) ? CmdSub.Name._char[7] : AT24CXX_Manager_NET.MeterID[2]);
        AT24CXX_Manager_NET.MeterID[3] =
            (((unsigned char)CmdSub.Name._char[8] != 0xFF) ? CmdSub.Name._char[8] : AT24CXX_Manager_NET.MeterID[3]);
        ...
        RunTimeNum =
            (RunTimeNum & 0xFFFF0000) | (U8_Connect_U8(CmdSub.Name._char[7], CmdSub.Name._char[8]) & 0x0000FFFF);
        EEprom_Write_Start((unsigned char*)&RunTimeNum,
                           EEPROM_OTHER_ADDRESS + (AT24CXX_Manager_NET.RunTimeBack_AddrNum * 4), 4);
        ResFlag = false;
...
uint8_t HY_USB_TTL_CheckBuff(char* RxBuf, int RxLen, uint8_t Now_LPUartx) {
    ...
    if (!(((uint8_t)RXDatabuff.Name._char[2] == 0xFF && (uint8_t)RXDatabuff.Name._char[3] == 0xFF &&
           (uint8_t)RXDatabuff.Name._char[4] == 0xFF && (uint8_t)RXDatabuff.Name._char[5] == 0xFF) ||
          (AT24CXX_Manager_NET.MeterID[2] == RXDatabuff.Name._char[4] &&
           AT24CXX_Manager_NET.MeterID[3] == RXDatabuff.Name._char[5]))) {
        return false;
    }
    ...
    CmdTempFlag          = (CmdTempFlag << 16 & 0xFFFF0000) |
                  (U8_Connect_U8(RXDatabuff.Name._char[12], RXDatabuff.Name._char[13]) & 0x0000FFFF);
    ...
    if (CmdTempFlag == CMD_CODE_UPDATA) {
        ...
        if (temp == 0) {
            ...
            SendDataLen += catString(&SendBuf.Name._char[SendDataLen], "{\"upgrades\":\"start\"}", (512 - SendDataLen),
                                     strlen("{\"upgrades\":\"start\"}"));
            RTC_TASK.InitSetTimeTask(HomePageRefresh, 0, ShowHomePage);
        } else if (temp == 1) {
            newString(ResStr, 60);
            ...
            SendDataLen += catString(&SendBuf.Name._char[SendDataLen], ResStr.Name._char, (512 - SendDataLen),
                                     ResStr.getStrlen(&ResStr));
        } else if (temp == 2) {
            newString(ResStr, 60);
            ...
            SendDataLen += catString(&SendBuf.Name._char[SendDataLen], "{\"upgrades\":\"over\"}", (512 - SendDataLen),
                                     strlen("{\"upgrades\":\"over\"}"));
        ...
        for (int NowSubLenAddr = 14; NowSubLenAddr < RXDataMaxLen; NowSubLenAddr = 14 + SteepAddr) {
            ...
            // 回复
            if ((CmdTempFlag == CMD_CODE_CTRL || CmdTempFlag == CMD_CODE_WRITE) &&
                (NowSubLenAddr + (SubLen + 5) > RXDataMaxLen)) {
                ...
                SendDataLen =
                    Add_HY_DataClass(NEW_NAME(SendBuf.Name._char), SendDataLen, RXDatabuff.Name._char[17 + SteepAddr],
                                     RXDatabuff.Name._char[18 + SteepAddr]);
            } else {
                SendBuf.Name._char[SendDataLen++] = 0x00;
...
void CheckMeterNum(void) {
    uint32_t RunTimeNum = 0;
    EEprom_Read_Start((unsigned char*)&RunTimeNum, EEPROM_OTHER_ADDRESS + (AT24CXX_Manager_NET.RunTimeBack_AddrNum * 4),
                      4);
