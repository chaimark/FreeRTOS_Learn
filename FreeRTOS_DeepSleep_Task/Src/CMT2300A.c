...
unsigned char CMT2300A_AutoSwitchStatus(unsigned char nGoCmd) {
    ...
    i = 10;
    while (i) {
        FL_DelayUs(100);//   CMT2300A_DelayUs(100);
        if (nWaitStatus == CMT2300A_GetChipStatus())return TRUE;
        if (CMT2300A_GO_TX == nGoCmd) {
            FL_DelayUs(100); //CMT2300A_DelayUs(100);
            if (CMT2300A_MASK_TX_DONE_FLG & CMT2300A_ReadReg(CMT2300A_CUS_INT_CLR1))return TRUE;
        }
        if (CMT2300A_GO_RX == nGoCmd) {
            FL_DelayUs(100); //CMT2300A_DelayUs(100);
            if (CMT2300A_MASK_PKT_OK_FLG & CMT2300A_ReadReg(CMT2300A_CUS_INT_FLAG))return TRUE;
        }
        i--;
        if (i == 0)return FALSE;
    }
...
void CMT2300A_SetInterruptPolar(unsigned char bActiveHigh) {
    ...
    if (bActiveHigh)tmp &= ~CMT2300A_MASK_INT_POLAR;
    else tmp |= CMT2300A_MASK_INT_POLAR;
...
void CMT2300A_EnableRxFifoAutoClear(unsigned char bEnable) {
    ...
    if (bEnable)tmp &= ~CMT2300A_MASK_FIFO_AUTO_CLR_DIS;
...
void CMT2300A_EnableFifoMerge(unsigned char bEnable) {
    unsigned char tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_CTL);
    if (bEnable) tmp |= CMT2300A_MASK_FIFO_MERGE_EN;
    else tmp &= ~CMT2300A_MASK_FIFO_MERGE_EN;
    CMT2300A_WriteReg(CMT2300A_CUS_FIFO_CTL, tmp);
}
...
void CMT2300A_EnableTxDin(unsigned char bEnable) {
    unsigned char tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_CTL);
    if (bEnable) tmp |= CMT2300A_MASK_TX_DIN_EN;
    else tmp &= ~CMT2300A_MASK_TX_DIN_EN;
    CMT2300A_WriteReg(CMT2300A_CUS_FIFO_CTL, tmp);
}
...
void CMT2300A_EnableTxDinInvert(unsigned char bEnable) {
    ...
    if (bEnable) tmp |= CMT2300A_MASK_TX_DIN_INV;
    else tmp &= ~CMT2300A_MASK_TX_DIN_INV;
...
void CMT2300A_EnableLfoscOutput(unsigned char bEnable) {
    unsigned char tmp = CMT2300A_ReadReg(CMT2300A_CUS_INT2_CTL);
    if (bEnable) tmp |= CMT2300A_MASK_LFOSC_OUT_EN;
    else tmp &= ~CMT2300A_MASK_LFOSC_OUT_EN;
    CMT2300A_WriteReg(CMT2300A_CUS_INT2_CTL, tmp);
}
...
unsigned char CMT2300A_ConfigRegBank(unsigned char base_addr, const unsigned char bank[], unsigned char len) {
    unsigned char i;
    for (i = 0; i < len; i++) CMT2300A_WriteReg(i + base_addr, bank[i]);
    return TRUE;
}
