...
void Valve_SetDegree(uint16_t Set_Degree_Part, ValveReasonsDone DoneId) {
    ...
    if ((System_RunData.Now_Degree_Part >=
         ((Set_Degree_Part < DegreePartToUint(PrecisionNum) ? 0 : Set_Degree_Part - DegreePartToUint(PrecisionNum)))) &&
    ...
    do {
        ...
        if (((System_RunData.Now_Degree_Part == 0) || System_RunData.Now_Degree_Part == DegreePartToUint(100)) &&
            ((Set_Degree_Part != 0) && (Set_Degree_Part != DegreePartToUint(100))) && (Temp_Max_Time_Count > 40)) {
            break;
        }
