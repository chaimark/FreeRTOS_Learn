...
const uint16_t MeterTemperature_Table[][10] = {
    {
        10000,
        10004,
        10008,
...
float TEMP_CAL(unsigned int value) {
    ...
    for (unsigned int i = 0; i < ARR_SIZE(MeterTemperature_Table); i++) {
        for (unsigned int j = 0; j < ARR_SIZE(MeterTemperature_Table[0]); j++) {
            unsigned int diff = (value > MeterTemperature_Table[i][j]) ? (value - MeterTemperature_Table[i][j])
                                                                       : (MeterTemperature_Table[i][j] - value);
            if (diff < min_diff) {
                min_diff  = diff;
                closest_i = i;
                closest_j = j;
...
void Test_PT1000(float* PT1000_T1, float* PT1000_T2) {
    ...
    if ((Current_meter_status == 0) || (Current_meter_status == 1)) {
        ...
        TEMP_R_VALUE_PT1 =
            (TEST_PT1000_T1_VALUE[1] + TEST_PT1000_T1_VALUE[2] + TEST_PT1000_T1_VALUE[3] + TEST_PT1000_T1_VALUE[4]) >>
            2;
        ...
        TEMP_R_VALUE_PT2 =
            (TEST_PT1000_T2_VALUE[1] + TEST_PT1000_T2_VALUE[2] + TEST_PT1000_T2_VALUE[3] + TEST_PT1000_T2_VALUE[4]) >>
            2;
