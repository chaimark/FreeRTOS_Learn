...
void WriteRunTime(uint16_t backNum) {
    ...
    if (RunTimeNum > 150000 * (backNum + 1)) { // 是否需要切换
        ...
        EEprom_AT24C0XXData_Write(&AT24CXX_Manager_NET.RunTimeBack_AddrNum,
                                  sizeof(AT24CXX_Manager_NET.RunTimeBack_AddrNum));
    } else { // 写阀门运行时间
...
void WriteEEprom(void* pvParameters) {
    ...
    while (1) {
        ...
        /* 永久阻塞，直到任意一个事件位被置位 */
        EventBits_t uxBits =
            xEventGroupWaitBits(xEventGroup_EEprom, AllxEvent_EEprom, pdTRUE, /* 退出前自动清除对应位 */
                                pdFALSE,                                      /* 任意一位即可唤醒 */
                                portMAX_DELAY);
        if (uxBits & WriteNowRTCTime) {
            ...
            EEprom_AT24C0XXData_Write(&AT24CXX_Manager_NET.UserSet_DegreePart,
                                      sizeof(AT24CXX_Manager_NET.UserSet_DegreePart));
            clearEEpromFlag;
