...
char readNowSendMode(int DayNumFormStartDay) {
    ...
    if ((StartDay[0] >= StartDay[1]) || (StartDay[1]) >= (StartDay[2])) {
        ...
        EEprom_AT24C0XXData_Write(&AT24CXX_Manager_NET.SendManageObj.SendFlagMode,
                                  sizeof(AT24CXX_Manager_NET.SendManageObj.SendFlagMode));
...
uint32_t InitSendModeAndTimeTask(void) {
    // ************************************ 判断当天属于什么模式 ************************************ //
    uint32_t Now_TimeToSec = anyBaseToAnyBase(NowHour, 10, 16) * 3600 + anyBaseToAnyBase(NowMinute, 10, 16) * 60 +
                             anyBaseToAnyBase(NowSecond, 10, 16);
    // 获取当前日距离1月1日的天数
    ...
    if (AT24CXX_Manager_NET.SendManageObj.SendStartHour > 23) {
        ...
        EEprom_AT24C0XXData_Write(&AT24CXX_Manager_NET.SendManageObj.SendStartHour,
                                  sizeof(AT24CXX_Manager_NET.SendManageObj.SendStartHour));
        IncludeDelayMs(200);
    }
    ...
    uint32_t TempSendStartHourToSec =
        (AT24CXX_Manager_NET.SendManageObj.SendStartHour * 3600) + TempMeterIdToSec; // 转换为秒
    if (Now_TimeToSec >= TempSendStartHourToSec) {
        if (Now_TimeToSec == TempSendStartHourToSec) {
            _Module_Start_Flage = _Module_AutoSend;
        }
        TempSendStartHourToSec =
            AT24CXX_Manager_NET.SendManageObj.SendIntervalDay -
            (Now_TimeToSec - TempSendStartHourToSec) % (AT24CXX_Manager_NET.SendManageObj.SendIntervalDay);
    } else {
        TempSendStartHourToSec =
            (TempSendStartHourToSec - Now_TimeToSec) % (AT24CXX_Manager_NET.SendManageObj.SendIntervalDay);
    }
    ...
    if (!DayNum) {
        if (NowHour > AT24CXX_Manager_NET.SendManageObj.SendStartHour -
                          (AT24CXX_Manager_NET.SendManageObj.SendIntervalDay / 3600)) {
        ...
        if (Now_TimeToSec >= ((AT24CXX_Manager_NET.SendManageObj.SendStartHour * 3600) + TempMeterIdToSec -
                              AT24CXX_Manager_NET.SendManageObj.SendIntervalDay)) {
            // 当前时间小于起始点
            RTC_TASK.CloseTask(SendIntervalTask); // 当天的任务强制关闭
            ...
            if (DayNum + 1 ==
                AT24CXX_Manager_NET.SendManageObj.SendFlagMode[NowSendFlagMode].SendInterval) { // 如果昨天刚传
