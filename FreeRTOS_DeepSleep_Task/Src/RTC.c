...
unsigned char Check_Date_ByHEX(unsigned char H_year1, unsigned char H_year2, unsigned char H_month,
                               unsigned char H_date) {
    unsigned int  w_year;
    ...
    if (w_date > 31)
        return 0;
    else if (w_month < 1 || w_date > Month_buf[w_month - 1] || w_date < 1)
        return 0;
    return 1;
}
...
void setRtcDate(strnew SteStrRTCData, char isWriteEEPROM) {
    ...
    sscanf(NowAddr, "%d-%d-%d %d:%d:%d", &SetRTC.year, &SetRTC.month, &SetRTC.day, &SetRTC.hour, &SetRTC.minute,
           &SetRTC.second);
    ...
    SetRTC.week =
        getDayOfWeek(SetRTC.year, SetRTC.month, SetRTC.day); // 计算周 周不需要转换, 因为 1-7 的 16进制 和 10进制 相同
...
void getStrUserTime(FL_RTC_InitTypeDef UserDate, char NowStrRTCData[]) {
    // 年月日时分秒
    sprintf(NowStrRTCData, "20%02x-%02x-%02x %02x:%02x:%02x", UserDate.year, UserDate.month, UserDate.day,
            UserDate.hour, UserDate.minute, UserDate.second);
}
/************************************************************************************/
